...
module.exports.config = {
	...
	version: "2.4.3",
	hasPermssion: 0,
	credits: "ProCoderMew & chỉnh font chữ by tnhan",
...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("[🐧] 𝗩𝘂𝗶 𝗹𝗼̀𝗻𝗴 𝗻𝗵𝗮̣̂𝗽 𝟭 𝗶𝗱/𝗹𝗶𝗻𝗸 𝗽𝗿𝗼𝗳𝗶𝗹𝗲 𝘂𝘀𝗲𝗿 𝗰𝗮̂̀𝗻 𝗮𝗱𝗱.");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("[🐧] 𝗞𝗵𝗼̂𝗻𝗴 𝘁𝗶̀𝗺 𝘁𝗵𝗮̂́𝘆 𝗜𝗗 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗱𝘂̀𝗻𝗴.")
			else {
				await adduser(id, name || "𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗱𝘂̀𝗻𝗴 𝗙𝗮𝗰𝗲𝗯𝗼𝗼𝗸");
			}
		} catch (e) {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`[🐧] ${name ? name : "𝗧𝗵𝗮̀𝗻𝗵 𝘃𝗶𝗲̂𝗻"} đ𝗮̃ 𝗰𝗼́ 𝗺𝗮̣̆𝘁 𝘁𝗿𝗼𝗻𝗴 𝗻𝗵𝗼́𝗺.`);
		else {
			...
			try {
				...
				return out(`[🐧] 𝗞𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝘁𝗵𝗲̂𝗺 ${name ? name : "𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗱𝘂̀𝗻𝗴"} 𝘃𝗮̀𝗼 𝗻𝗵𝗼́𝗺.`);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`[🐧] Đ𝗮̃ 𝘁𝗵𝗲̂𝗺 ${name ? name : "𝘁𝗵𝗮̀𝗻𝗵 𝘃𝗶𝗲̂𝗻"} 𝘃𝗮̀𝗼 𝗱𝗮𝗻𝗵 𝘀𝗮́𝗰𝗵 𝗽𝗵𝗲̂ 𝗱𝘂𝘆𝗲̣̂𝘁!`);
			else return out(`[🐧] Đ𝗮̃ 𝘁𝗵𝗲̂𝗺 ${name ? name : "𝗧𝗵𝗮̀𝗻𝗵 𝘃𝗶𝗲̂𝗻"} 𝘃𝗮̀𝗼 𝗻𝗵𝗼́𝗺 !`)
		}
	}
}